...
void main()
{
	...
	v_normal = normalize((model_transformed * vec4(normal, 0.0f))).xyz;
	v_position = (model_transformed * vec4(position, 1.0f)).xyz;
	v_normal_space = (mat3(
		normalize((model_transformed * vec4(bi_tangent, 0.0f))).xyz,
		normalize((model_transformed * vec4(tangent, 0.0f))).xyz,
		normalize((model_transformed * vec4(normal, 0.0f))).xyz
	));

	v_shadow_tex = offset_tex * shadow_projection * shadow_view * model_transformed * vec4(position, 1.0f);
